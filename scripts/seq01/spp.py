...
class SeqMod(nn.Module):
    def __init__(self, modname, poolsize=(1, 2, 6), dense_units = 64, ratio = 8, \
                 lstm_layers = 2):
        # Only resnet is supported in this version
        ...
        self.conv_first = nn.Sequential(nn.Conv1d(self.embed_size, self.dense_units*2*self.ratio, \
                                        ...
                                        nn.BatchNorm1d(self.dense_units*2*self.ratio),
                                        nn.ReLU(),
                                        nn.Conv1d(self.dense_units*2*self.ratio, self.dense_units*self.ratio, \
                                                  kernel_size=3, stride=1, padding=2,dilation=2, bias=False),
                                        nn.BatchNorm1d(dense_units*ratio),
                                        nn.ReLU())
        self.conv_res = nn.Sequential(nn.Conv1d(self.dense_units * self.ratio, self.dense_units * self.ratio, \
                                                kernel_size=3, stride=1,padding=4,dilation=4, bias=False),
                                      nn.BatchNorm1d(self.dense_units * self.ratio),
                                      nn.ReLU(),
                                      nn.Conv1d(self.dense_units * self.ratio, self.dense_units * self.ratio, \
                                                kernel_size=3, stride=1,padding=2,dilation=2, bias=False),
                                      nn.BatchNorm1d(dense_units * ratio),
                                      nn.ReLU())
        self.conv_final = nn.Sequential(nn.Conv1d(self.dense_units*self.ratio, self.dense_units, \
        ...
        self.lstm = nn.GRU(self.dense_units * self.ratio, self.dense_units, \
                           num_layers=self.lstm_layers, batch_first=True, bidirectional=True)
        self.final = nn.Sequential(nn.Conv1d(self.dense_units*2, 1, \
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size * seqlen, *x.size()[2:])
        # Pass each frame thru SPPNet
        emb = self.modbase.sppnet(x.permute(0,3,1,2))
        # Split back out to batch
        emb = emb.view(batch_size, seqlen, emb.size()[1])
        # Conv1d part
        ...
        avg_pool = torch.mean(x_cnn, 2)
        max_pool, _ = torch.max(x_cnn, 2)
        h_conc = torch.cat((max_pool, avg_pool), 1)
        h_conc_linear1  = F.relu(self.linear1(h_conc))
        h_conc_linear2  = F.relu(self.linear2(h_conc))
        hidden = h_conc + h_conc_linear1 + h_conc_linear2
        out = self.linear_out(hidden)
        # Bidirectional GRU part
