...
parser.add_option('-s', '--maxlen', action="store", dest="maxlen", help="accumulation steps", default="32")
...
class SPPSeqNet(nn.Module):
    def __init__(self, backbone, embed_size, pool_size=(1, 2, 6), pretrained=True, \
                 dense_units = 256, dropout = 0.4):
        # Only resnet is supported in this version
        super(SPPSeqNet, self).__init__()
        self.sppnet = SPPNet(backbone=50, pool_size=pool_size, folder=WTSPATH)
        self.dense_units = dense_units
        self.lstm1 = nn.LSTM(embed_size, self.dense_units, bidirectional=False, batch_first=True)
        self.linear1 = nn.Linear(self.dense_units, self.dense_units)
        self.linear_out = nn.Linear(self.dense_units, 1)
...
poolsize=(1, 2, 6)
embedsize = 4*512*sum(i**2 for i in poolsize)
