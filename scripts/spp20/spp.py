...
class SPPSeqNet(nn.Module):
    def __init__(self, backbone, embed_size, pool_size=(1, 2, 6), pretrained=True, \
                 dense_units = 256, dropout = 0.2, mask_val = -9.99999):
        ...
        self.mask_val = torch.tensor([mask_val])
    ...
    def forward(self, x, mask):
        ...
        # Mask the output according to pad
        h_lstm1 = h_lstm1.where(mask[:, :, None]==1, self.mask_val.half().to(device))
        # Pool the results
