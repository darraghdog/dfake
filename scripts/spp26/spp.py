...
class SPPSeqNet(nn.Module):
    def __init__(self, backbone, embed_size, pool_size=(1, 2, 6), pretrained=True, \
        ...
        self.linear1 = nn.Linear(self.dense_units*2, 1)
    ...
    def forward(self, x):
        ...
        # Pass batch thru sequential model(s) for video level prediction
        h_lstm1, _ = self.lstm1(emb)
        max_pool, _ = torch.max(h_lstm1, 1)
        h_pool_linear = F.relu(self.linear1(max_pool))
        #logger.info(h_lstm1.shape)

        # Get the frame level prediction
        outseq = self.linear1(h_lstm1)
...
def collatefn(batch):
    ...
    # Create a mask
    mask = torch.zeros((x_batch.shape[0], maxlen))
    for i, s in enumerate(seqlen): 
        mask[i, :s] = 1.
    
    if 'labels' in batch[0]:
        y_batch = torch.tensor([l['labels'] for l in batch])
        return {'frames': x_batch, 'ids': ids, 'seqlen': seqlen, 'mask': mask, 'labels': y_batch}
    else:
        return {'frames': x_batch, 'ids': ids, 'seqlen': seqlen, 'mask': mask}
...
for epoch in range(EPOCHS):
    ...
    if INFER not in ['TST', 'EMB', 'VAL']:
        ...
        for step, batch in enumerate(trnloader):
            ...
            m = batch['mask'].to(device, dtype=torch.float)
            sl = batch['seqlen'].to(device, dtype=torch.float)
            y = batch['labels'].to(device, dtype=torch.float)
            x = torch.autograd.Variable(x, requires_grad=True)
            y = torch.autograd.Variable(y)
            y = y.unsqueeze(1)
            out, outframe = model(x)
            # Get mean frame logit with mask
            outframe = ((outseq.squeeze(-1)*torch.tensor(m)).sum(-1) / sl).unsqueeze(1)           
            # Get loss on video
            loss = 0.6 * criterion(out, y) + 0.5 * criterion(outframe, y)
            tr_loss += loss.item()
