    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.train and (vid.label==1) and (random.randint(0,4)<3):
                ...
                ascdf = metadf[ metadf.original == metadf.query('video == @vid').original.iloc[0]]
                ascdf = ascdf[['video', 'original']].drop_duplicates().reset_index(drop=True)
                ascls = [samp] + ascdf.sample(min(3, ascdf.shape[0])).video.tolist() + \
                ...
                if (SKIP>1) and (framesdict[samp].shape[0] > SKIP*6):
                    every_k = random.randint(0,SKIP-1)
                    for t,b in enumerate(framesdict[samp]):
                        if t%SKIP!=every_k:
                             continue
                        try:
