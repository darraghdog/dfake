...
class SPPSeqNet(nn.Module):
    def __init__(self, backbone, embed_size, pool_size=(1, 2, 6), pretrained=True, \
        ...
        self.linear_out1 = nn.Linear(self.dense_units*2, 1)
        self.linear2 = nn.Linear(self.dense_units*2, self.dense_units*2)
        self.linear_out2 = nn.Linear(self.dense_units*2, 1)
    ...
    def forward(self, x):
        ...
        h_pool_linear1 = F.relu(self.linear1(max_pool1))
        max_pool2, _ = torch.max(h_lstm1, 1)
        h_pool_linear2 = F.relu(self.linear2(max_pool2))
        
        # Max pool and linear layer
        hidden1 = max_pool1 + h_pool_linear1
        hidden2 = max_pool2 + h_pool_linear2
        
        # Classifier
        out = self.linear_out1(hidden1)
        # Pixel change
        outpix = self.linear_out2(hidden2)
        return out, outpix
...
class DFakeDataset(Dataset):
    def __init__(self, df, imgdir, framels, aug_ratio = 5, train = False, val = False, labels = False, maxlen = 32 // SKIP):
        self.data = df.copy()
        logger.info(self.data.label.value_counts())
        #logger.info(self.data.head(20))
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            masks[masks<10] = 0
            pixchg = np.log(1+ masks.mean((0,1,2)))
            ...
            if self.train:
                ...
                pixchg = torch.tensor(pixchg)
                #logger.info(f'Labels {labels} Pixel change {pixchg}')
                return {'frames': frames, 'idx': idx, 'labels': labels, 'pixchg': pixchg}    
            else:      
...
def collatefn(batch):
    ...
    if 'labels' in batch[0]:
        ...
        y_pixchg = torch.tensor([l['pixchg'] for l in batch])
        return {'frames': x_batch, 'ids': ids, 'seqlen': seqlen, 'labels': y_batch, 'pixchg': y_pixchg }
    else:
...
for epoch in range(EPOCHS):
    ...
    if INFER not in ['TST', 'EMB', 'VAL']:
        ...
        for step, batch in enumerate(trnloader):
            ...
            px = batch['pixchg'].to(device, dtype=torch.float)
            #logger.info(y)
            #logger.info(px)
            ...
            out, outpix = model(x)
            # Get loss
            losscls, losspix = criterion(out, y), criterionpix(outpix, px)
            loss = 0.1 * losspix + 0.9 * losscls
            tr_loss += loss.item()
            p_loss += losspix.item()
            c_loss += losscls.item()
            ...
            if step % ACCUM == 0:
                ...
                optimizer.zero_grad()
            if step%100==0:
                logger.info('Trn step {} of {} trn lossavg {:.5f} pix {:.5f} cls {:.5f}'. \
                        format(step, len(trnloader), (tr_loss/(1+step)), (p_loss/(1+step)),(c_loss/(1+step))  ))
            del x, y, out, batch
