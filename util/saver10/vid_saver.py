...
parser.add_option('-n', '--reduction', action="store", dest="reduction", help="Reduction in size", default="1")
...
for tt, batch in enumerate(allloader):
    ...
    try:
        ...
        H, W = H//2, W//2
        imgls = [cv2.resize(i, dsize=(W, H), interpolation=cv2.INTER_LINEAR) for i in imgls]
        ...
        if len(trackmat)<5 and max(H,W)<2000:
            ...
            if len(trackmat)<5 and max(H,W)<2000:
                imgls, H, W = vid2imgls(VNAME, FPS, 20)
                H, W = H//2, W//2
                imgls = [cv2.resize(i, dsize=(W, H), interpolation=cv2.INTER_LINEAR) for i in imgls]
                trackmat, faces = gettrack(imgls, FPS//8, H, W, BATCHSIZE)
        ...
        try:
            ...
            logger.info('No objects found in video ... aaahhhh :(')
            continue
        END = datetime.datetime.now()
        N_OBJ, N_FACES = 0, 0
        STATUS = 'fail'
