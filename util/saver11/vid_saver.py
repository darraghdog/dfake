...
# OUTDIR = os.path.join(INPATH, 'data/npimg11')
# FPS = 8
# VIDPATH = 'data/train_sample_videos'
# BATCHSIZE=4
# MAXLOADSECONDS=20
...
metadf['video_path'] = os.path.join(INPATH, VIDPATH) + '/' + metadf['folder'] + '/' + metadf['video']
logger.info('Full video file shape {} {}'.format(*metadf.shape))

metadf = metadf.query('fold == @FOLD')
# metadf = metadf[metadf.original.isin(os.listdir(os.path.join(INPATH, VIDPATH)))]
...
for tt, VNAME in enumerate(VIDFILES):
    ...
    try:
        ...
        trackvid['video'] = VNAME.split('/')[-1]
        trackvid['maxdim'] = pd.concat([trackvid.x2-trackvid.x1, 
                trackvid.y2-trackvid.y1], axis=1).max(axis=1)
        outlieridx = np.abs(stats.zscore(trackvid['maxdim'].values))<2 # get rid of any outlier boxes
        trackvid['boxdim'] = BOXDIM = trackvid['maxdim'][outlieridx].max()
        ...
        for (t, row) in trackvid.iterrows():
            ...
            face = cv2.resize(face, (BOXDIM, BOXDIM), interpolation=cv2.INTER_CUBIC)
