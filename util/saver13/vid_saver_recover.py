...
logger.info('Len of total videos : {}'.format(len(VIDFILES)))
RECOVER=True
if RECOVER:
    compls = set([f.replace('.npz', '.mp4') for f in os.listdir(OUTDIR)])
    logger.info('len vidfiles {}'.format(len(VIDFILES)))
    VIDFILES = [v for v in VIDFILES if v.split('/')[-1] not in compls ]
    logger.info('len vidfiles {}'.format(len(VIDFILES)))
#VIDFILES = [v for v in VIDFILES if v.split('/')[-1] in set(['maaisyozrr.mp4', 'jquqnmdhpz.mp4', 'surqcuurvj.mp4']) ]
logger.info('Len of videos to recover : {}'.format(len(VIDFILES)))
#logger.info(VIDFILES[:2])
...
for tt, VNAME in enumerate(VIDFILES):
    ...
    try:
        ...
        for (t, row) in trackvid.iterrows():
            obj = row.obj
            try:
                frame = cv2.copyMakeBorder( imgls[int(row.frame)].copy(), B, B, B, B, cv2.BORDER_CONSTANT)
                ...
                logger.info('Error rolling : {}'.format(VNAME.split('/')[-1]))
...
logdf.to_csv(os.path.join(OUTDIR, 'log_fold{}_{}.txt'.format(FOLD,  datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")   )), index = False)
trackdf.to_csv(os.path.join(OUTDIR, 'tracker_fold{}_{}.txt'.format(FOLD, datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S"))), index = False)
