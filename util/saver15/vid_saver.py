...
metadf['original_path'] = os.path.join(INPATH, options.vidpath) + '/' + metadf['folder'] + '/' + metadf['original']
...
trkdf = pd.read_csv(trknms[0])
trkdf.drop_duplicates('video').frame.value_counts().sort_index()
'''
...
# WTSFILES=os.path.join(INPATH, 'weights')
...
VNAME = tmpls[0]
"errocgcham.mp4":{"label":"FAKE","split":"train","original":"grnycmbdfu.mp4"}
vdict[VNAME] = tmpls[1:]
FPS=8
MAXLOADSECONDS=10
SIZE=224
OUTDIR=os.path.join(INPATH, 'data')
VNAME="/Users/dhanley2/Documents/Personal/dfake/data/train_sample_videos/errocgcham.mp4"
vdict[VNAME]=["/Users/dhanley2/Documents/Personal/dfake/data/train_sample_videos/grnycmbdfu.mp4"]
'''

for tt, VNAME in enumerate(ORIGFILES):
    ...
    try:
        ...
        thumbls, h, w = imresizels(imgls, H, W, MAXDIM = 768)
        ...
        for (t, row) in trackvid.iterrows():
            ...
            oimgdict[obj].append(face)
        otrackfaces = np.array(sum(list(oimgdict.values()), []))
        ...
        for FAKENAME in vdict[VNAME]:
            ...
            fimgdict = collections.OrderedDict((o, []) for o in range(1+trackvid.obj.max()))         
            for (t, row) in trackvid.iterrows():
                obj = row.obj
                frame = imgls[row.frame]
                face = frame[row.y1:row.y1+row.maxdim, row.x1:row.x1+row.maxdim]
                face = cv2.resize(face, (SIZE,SIZE), interpolation=cv2.INTER_CUBIC)
                fimgdict[obj].append(face)
